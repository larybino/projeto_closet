      ...
      child: ListView(
        ...
        children: [
            ...
            currentAccountPicture: GestureDetector(
              onTap: () {
                ...
                Navigator.pushNamed(
                  context,
                  Rotas.usuario,
                  arguments: _usuarioAtual,
                );
              },
              child: CircleAvatar(
                backgroundImage:
                    (_usuarioAtual.fotoPerfil != null &&
                            _usuarioAtual.fotoPerfil!.isNotEmpty)
                        ? NetworkImage(_usuarioAtual.fotoPerfil!)
                        : null,
                backgroundColor: Colors.white,
                child:
                    (_usuarioAtual.fotoPerfil == null ||
                            _usuarioAtual.fotoPerfil!.isEmpty)
                        ? Text(
                          _usuarioAtual.nome?.substring(0, 1).toUpperCase() ??
                              'U',
          ...
          _criarItemMenu(
            icone: Icons.home,
            rotulo: 'Início',
            aoPressionar: () => Navigator.pushNamed(context, Rotas.menu, arguments: _usuarioAtual),
          ),
          const Divider(),
          _criarItemMenu(
            icone: Icons.person,
            ...
            aoPressionar: () async {
              final usuarioAtualizado = await Navigator.push<DTOUsuario>(
                ...
                MaterialPageRoute(
                  builder:
                      (context) => WidgetEditarPerfil(usuario: _usuarioAtual),
                ),
              );
